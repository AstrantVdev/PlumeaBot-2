...
export abstract class Item {
     ...
     */
    async create(what: Object=null): Promise<void>{
...
export async function setUp(): Promise<void>{
    ...
    Item.db = new Sequelize(process.env.DB, process.env.DB_USER, process.env.DB_PASS, {
...
export async function sync(): Promise<void>{
    const files = getAllFilesInDir("db/dbObs")

    for(const file of files){
        const object: Item = new (require(file)[file.split("/")[-1].slice(0, -3)])()
        object.tab.sync()
    }
